...
用GoogleNet作为目标模型，数据样本大小为500，获得了30%的fooling rate。作者观察到使用仅有的数据无法达到使用全部数据的攻击性能。作者认识到主要原因是攻击成功率的减少是因为对有限数据的过拟合。一个方法是进行数据增强（随机旋转（5个角度）、随机 裁剪、随机水平或垂直翻转或）。表4显示了结果。
